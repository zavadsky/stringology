...
int Zk_byte_w3_nop(unsigned char *x, const int m, unsigned char *y, int n,int k=13) {
	int mp1=m+1, mm1=m-1, mm2=m-2, mm3=m-3, m2=m*2, r, count = 0,outs=0;
	unsigned int third = n/3, twothird = 2*n/3, fin=n, pos1, pos2, pos3, stop;
...
// Preprocessing
    ...
    if(m>2) {
        while (pos1<third) {
            ...
            while(Z[((*(unsigned short*)(y+pos1)))&mask] & Z[((*(unsigned short*)(y+pos2)))&mask] & Z[((*(unsigned short*)(y+pos3)))&mask]){
                pos1+=mm1;  // Alg. 3, line 6
                pos2+=mm1;
                pos3+=mm1;
            }
            if(!Z[(*(unsigned short*)(y+(--pos1)))&mask]) {         // Alg. 3, line 7 combined with Alg. 5, line 7
                for (r = 0; r < m && y[pos1+r-mm3] == x[r]; r++);	// occurrence check, Alg. 3, line 9
                if (r == m)
                    if(pos1<third)
                        count++;
                pos1+=QS[y[pos1+3]]+1;  // Alg. 3, line 10 + Alg. 3, line 4
            } else
                pos1+=mm1;              // Alg. 3, line 4
            if(!Z[(*(unsigned short*)(y+(--pos2)))&mask]) {         // Alg. 3, line 7 combined with Alg. 5, line 12
                for (r = 0; r < m && y[pos2+r-mm3] == x[r]; r++);	// occurrence check, Alg. 3, line 9
                if (r == m)
                    if(pos2<twothird)
                        count++;
                pos2+=QS[y[pos2+3]]+1;  // Alg. 3, line 10 + Alg. 3, line 4
            } else
                pos2+=mm1;              // Alg. 3, line 4
            if(!Z[(*(unsigned short*)(y+(--pos3)))&mask]) {
                for (r = 0; r < m && y[pos3+r-mm3] == x[r]; r++);	// occurrence check, Alg. 3, line 9
                if (r == m)
                    if(pos3<fin)
                        count++;
                pos3+=QS[y[pos3+3]]+1;  // Alg. 3, line 10 + Alg. 3, line 4
            } else
                pos3+=mm1;              // Alg. 3, line 4
        }
        while(pos2<twothird) {          // If pos1 has reached 1/3 of the text, but pos2 has not reached 2/3 of the text
            while(Z[((*(unsigned short*)(y+pos2)))&mask])
                pos2+=mm1;
            for (r = 0; r < m && y[pos2+r-mm2] == x[r]; r++);
            if (r == m)
                if(pos2<twothird)
                    count++;
            pos2+=QS[y[pos2+2]];
        ...
        while(pos3<fin) {               // If pos1 has reached 1/3 of the text, but pos3 has not reached the end of the text
            while(Z[((*(unsigned short*)(y+pos3)))&mask])
                pos3+=mm1;
            for (r = 0; r < m && y[pos3+r-mm2] == x[r]; r++);
            if (r == m)
                if(pos3<fin)
                    count++;
            pos3+=QS[y[pos3+2]];
            ...
            while (pos1<third) {
                ...
                if(!Z[(*(unsigned short*)(y+pos1))&mask]) // occurrence check
                    if(pos1<third)
                        count++;
                pos1+=QS[y[pos1+2]];
                if(!Z[(*(unsigned short*)(y+pos2))&mask]) // occurrence check
                    if(pos2<twothird)
                        count++;
                pos2+=QS[y[pos2+2]];
                if(!Z[(*(unsigned short*)(y+pos3))&mask]) // occurrence check
                    if(pos3<fin)
                        count++;
                pos3+=QS[y[pos3+2]];
            }
